pipeline{
    ...
    stages{
        ...
        stage("Checkout from SCM"){
            steps {
                git branch: 'main', credentialsId: 'dockerhub', url: 'https://github.com/Ajoke93/complete-prodcution-e2e-pipeline.git'
            }
        ...
        stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'https://jenkins.keyatcloud.com/job/gitops-complete-pipeline/buildWithParameters?token=gitops-token'"
                }
            }
